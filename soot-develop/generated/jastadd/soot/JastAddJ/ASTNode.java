...
public class ASTNode<T extends ASTNode> extends beaver.Symbol  implements Cloneable, Iterable<T> {
   ...
   * @apilevel internal
   */
  protected static ASTNode_State state = new ASTNode_State();
  /**
   * @apilevel internal
   * @ast method 
   * 
   */
  public final ASTNode_State state() { return state; }
  /**
   * @apilevel internal
   * @ast method 
   * 
  ...
  @SuppressWarnings("cast") public T getChild(int i) {
    ...
    if(!node.in$Circle()) {
      ...
      do {
        this.state().push(ASTNode_State.REWRITE_CHANGE);
      ...
      } while(rewriteState == ASTNode_State.REWRITE_CHANGE);
      if(rewriteState == ASTNode_State.REWRITE_NOCHANGE && this.is$Final()) {
        node.is$Final(true);
  ...
  protected boolean duringImplicitConstructor() {
    if(state().duringImplicitConstructor == 0) {
      ...
      state().pop();
      state().push(ASTNode_State.REWRITE_INTERRUPT);
      return true;
    }
  }
  /**
  ...
  public ASTNode rewriteTo() {
    if(state().peek() == ASTNode_State.REWRITE_CHANGE) {
      state().pop();
      state().push(ASTNode_State.REWRITE_NOCHANGE);
    }
    return this;
  }
  /**
